...
- `content`: one text slide at a time. A slide may contain a title, short body, optional semantic icon, short list and link. Use `count`, `dots` or `count-and-dots` indicators. `surfaceTreatment` is limited to `plain` (the default), `single-surface` or `varied-surfaces`; authors cannot supply arbitrary colours.
...
- `gallery`: one image with an optional title, Body description and genuine Caption at a time. It can use counts, dots or accessible thumbnail buttons. Only this variant supports `controls="overlay"`; its controls use the shared icon treatment on translucent protective surfaces.
