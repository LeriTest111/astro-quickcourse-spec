...
Content structure or props: a stable group `id`, optional heading and intro, items with a stable id, title, normal text content, optional short label or number, optional meaningful icon, optional image media, list or small callout. Use controlled `behavior` (`multiple` or `single`), `initialOpen` (`none`, `first` or an item id), `orientation` (`vertical` or `horizontal`), `presentation` (`minimal`, `contained` or `surface`), cohesive `surface` (`neutral`, `accent` or `information`), `headingScale` (`compact`, `standard` or `prominent`) and `density` (`compact` or `comfortable`).

Interaction behaviour: the vertical system uses native `details` and `summary`. Multiple-open is the default. Single-open uses native named disclosure groups with a small scoped JavaScript fallback that closes siblings; an open item can always be closed. Progressive enhancement adds a subtle 220ms height and opacity transition on both open and close. The dedicated right-side disclosure arrow rotates on open. Meaningful left-side icons remain stationary.
...
Responsive behaviour: vertical content grows naturally and does not clip long titles or panel content. Image-and-text panels use a generous desktop gap and stack on smaller screens. Horizontal image-led accordions require three to five concise items and single-open behaviour; inactive desktop items show only their image and concise title, while the active item reveals detail. They become normal vertical accordions with panel images on narrow screens.

Accessibility considerations: native disclosure semantics communicate state. Summary text must be meaningful; colour is not the only open-state indicator; media needs appropriate alt text; focus remains visible; and reduced-motion makes panel and arrow state changes effectively immediate. Printed output exposes all panel content.
